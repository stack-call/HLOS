...
Image: boot/bootsect boot/setup tools/system init/main.o
	...
	dd if=tools/system of=hd60M.img bs=512 count=1000 seek=5 conv=notrunc

	gcc ./test/test64k.S -m32 -c -o test64k.o
	$(OBJCOPY) $(OBJCOPYFLAGS) test64k.o test64k.bin
	dd if=test64k.bin of=hd60M.img bs=512 count=128 seek=140 skip=0 conv=notrunc
#dd if=test64k.bin of=hd60M.img bs=512 count=128 seek=140 skip=0 conv=notrunc
